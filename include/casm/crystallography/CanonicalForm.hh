...
template <typename ExternSymOpVector>
std::vector<Lattice> equivalents(Lattice const &lat,
                                 ExternSymOpVector const &point_grp,
                                 double compare_tol) {
  return equivalents(
      lat, adapter::Adapter<SymOpVector, ExternSymOpVector>()(point_grp),
      compare_tol);
}

/// Distinct symmetrically equivalent lattices, using the provided group and
/// this lattice's tolerance
std::vector<Lattice> equivalents(const Lattice &lat,
                                 SymOpVector const &point_grp);
