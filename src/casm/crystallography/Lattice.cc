...
//*******************************************************************************************
//
//  Finds the Displacement Shift Complete (DSC) lattice L_{dsc}, the coarsest
//  lattice whose set of lattice points contains all points of both L_1 and L_2.
//  Equivalently, L_{dsc} is the group generated by L_1 ∪ L_2.
//
//  This is the dual of make_superduperlattice: where that function finds the
//  finest common superlattice (L_1 ∩ L_2), this finds the coarsest common
//  containing lattice (L_1 + L_2). The two are related by
//  vol(L_{dsc}) * vol(L_{sd}) = vol(L_1) * vol(L_2).
//
//  Algorithm: dA = inv(L_2)*L_1 = A/n, where A is integer and n is the LCM of
//  denominators. Smith normal form A = U*S*V. G(i,i) = gcd(S(i,i), n).
//  Then L_{dsc} = L_2 * U * G / n.
//
//*******************************************************************************************
Lattice make_itsybitsylattice(const Lattice &lat1, const Lattice &lat2) {
  ...
  // reuse matrix S for matrix 'G', G(i,i) = gcd(S(i,i), N)
  for (Index i = 0; i < 3; i++) {
    S(i, i) = gcf(S(i, i), N);
  }
  // L_{dsc} = lat2 * U * G / N

  Lattice tlat(lat2.lat_column_mat() * (U * S).cast<double>() / double(N));
  return tlat.reduced_cell();
}
